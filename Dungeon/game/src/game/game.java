...
public class game extends JPanel {
   ...
   BufferedImage playerImage, playerL, playerR, enemies, tileset, items, title;
   ...
   public game() {
      ...
      //theses two listeners let the game play only when it is in focus
         ...
         public void keyPressed(KeyEvent evt) {
            ...
            if (code == KeyEvent.VK_LEFT) {
              ...
              player.newPosition(0,1, map.mapLayout);
            	...
            	if (map.mapLayout[player.currentX][player.currentY].type == "key") {
            	...
            	//picking up the final treasure
            	if (map.mapLayout[player.currentX][player.currentY].type == "end") {
            		gameWon = true;
            	}
            }
         }
      ...
      try {
      	   playerR = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/KnightR.png" ));
      	   playerL = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/KnightL.png" ));
      	   enemies = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/enemies.png" ));
      	   tileset = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/tile.png" ));
      	   items = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/items.png" ));
      	   title = ImageIO.read(new File("C:/Users/tibbs/Desktop/game20253079/titlesAndText.png" ));
      	   playerImage = playerR;
      	} catch (IOException e) {
      	    e.printStackTrace();
      	};
   ...
   private class Player {
	    	  ...
	    	  if (moving == false && lives>=0 && gameStarted == true)
	    	  {
		    	...
		    	//if not possible, check and then attack if it is an enemy
		    	  if (currentX+newX >=0 && currentX+newX <layout.length && currentY+newY >=0 && currentY+newY <layout[0].length && layout[currentX+newX][currentY+newY].type == "enemy" 
		    			  && newX+newY !=0 && player.moving ==false){
	    ...
	    //hop for feedback effect
	      ...
	      void draw(Graphics g) {  // Draws the player at its current location.
	         ...
	         g.drawString("H: "+party[currentParty][0]+"S: "+party[currentParty][4], (startX)+(cellSize*currentX)-moveX, (startY)+(cellSize*currentY)-moveY-hop);
   ...
   private class Room {
	      int currentX, currentY, shadeDir;  // Current room position.
	      ...
	      void draw(Graphics g) {  // Draws the room contents at its current location.
	    	  if (revealed == true && type != "hall") {
	    		  ...
	    		  //drawing enemies
		    	  if (type == "enemy" ) {
		    		  if (enemy.enemyLevel == 4) {subSprite = enemies.getSubimage(127,0,19,27);}
		    		  if (enemy.enemyLevel == 3) {subSprite = enemies.getSubimage(95,0,19,27);}
		    		  if (enemy.enemyLevel == 2) {subSprite = enemies.getSubimage(38+(19*(enemy.attack[0]+enemy.attack[1])),0,19,27);}
		    		  if (enemy.enemyLevel == 1) {subSprite = enemies.getSubimage(19*enemy.weaponType,0,19,27);}
			        	 g.drawImage(subSprite, (42+cellSize*currentX), (16+cellSize*currentY), 48, 60, null );
		    	  }
		    	  ...
		    	  for (int j = 0; j<mapLayout[0].length; j++) {
			    	  if (mapLayout[i][j].type != "null") {//show the map
				         ...
				         g.drawImage(tile, centreX+(cellSize*i)+6, centreY+(cellSize*j)+6, 42, 42, null);
				          ...
				          if (mapLayout[i][j].doors[1] == 1) {g.drawImage(door2, centreX+(cellSize*i), centreY+(cellSize*j), 48, 18, null);}
				          if (mapLayout[i][j].doors[3] == 1) {g.drawImage(door2, centreX+(cellSize*i), centreY+(cellSize*j)+37, 48, 11, null);}
		    	  ...
		    	  for (int j = 0; j<mapLayout[0].length; j++) {
			    	  if (mapLayout[i][j].type != "null") {//draw shadows on the shaded rooms, must be drawn over map in separate loop
			    		 if (mapLayout[i][j].shaded == true ) {
			    			  g.drawImage(shade, centreX+(cellSize*i)+6, centreY+(cellSize*j)+5, 42, 42, null);
					          if (mapLayout[i][j].doors[0] == 1) { g.drawImage(shadedoor1, centreX+(cellSize*i)-6, centreY+(cellSize*j), 25, 48, null);}
					          if (mapLayout[i][j].doors[1] == 1) {g.drawImage(shadedoor2, centreX+(cellSize*i), centreY+(cellSize*j)-4, 48, 24, null);}
					          if (mapLayout[i][j].doors[2] == 1) {g.drawImage(shadedoor1, centreX+(cellSize*i)+32, centreY+(cellSize*j), 24, 48, null);}
					          if (mapLayout[i][j].doors[3] == 1) {g.drawImage(shadedoor2, centreX+(cellSize*i), centreY+(cellSize*j)+33, 48, 24, null);}
